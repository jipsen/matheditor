...
var manageTextarea = (function() {
  ...
  return function manageTextarea(el, opts) {
    ...
    function popText(callback) {
      var text = textarea.val(); // PJ removed trim
      textarea.val('');          // removed space (now space works in textbox)
...
var RootMathBlock = P(MathBlock, function(_, _super) {
  ...
  _.onKey = function(key, e) {
    ...
    case 'Enter': 
	if (this === this.cursor.root) return false; // PJ allow return in text
...
CharCmds[' '] = bind(VanillaSymbol, '\\:', ' ');//PJ
...
//      alert("hi");
                var pageX = offset(self).left;
                self.insAtRightEnd(self);
                self.seekHoriz(pageX, self);
        }
